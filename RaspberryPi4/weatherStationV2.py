        ...
        while rx.ready():
            ...
            if msgPart0 == serial:
                ...
                if (time.time() - outsideDataLastSendTime) > 600:
                    ...
                    cursor.execute("INSERT INTO Global_Data (id, data_DateTime, temperature, humidity, pressure, windSpeed, maxWindSpeed, windDirection, battery) VALUES (NULL, %s, %s, %s, %s, %s, %s, %s, %s)", (time.strftime("%Y-%m-%d %H:%M:%S"), boardRecieveMsg[1], boardRecieveMsg[2],seaLevelPressure, boardRecieveMsg[4], maxWind, boardRecieveMsg[6], boardRecieveMsg[7]))#global data
                    WeatherDataBase.commit()
