...
try:
    while True:
        ...
        if sensor.get_sensor_data():
            ...
            actualInDataLastSendTime = datetime.now()
        else :
        ...
        if insideDataReady:
            change = True
            cursor.execute("UPDATE Weather_insideactualdata SET time = %s, temperature = %s, humidity = %s, pressure = %s WHERE id = 1", (datetime.utcnow(), insideTemperature, insideHumidity, seaLevelPressure))
        if outsideDataReady:
            change = True
            cursor.execute("UPDATE Weather_actualdata SET time = %s, temperature = %s, humidity = %s, pressure = %s, windSpeed = %s, average_wind_speed = %s, max_wind_speed_10min = %s, wind_direction = %s, battery = %s WHERE id = 1", (time.strftime("%Y-%m-%d %H:%M:%S"), outTemperature, outHumidity, outAverageWindSpeed, outWindSpeed, maxWind, outWindDirection, outBattery))
        if change:
